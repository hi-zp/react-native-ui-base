...
type PathArray<S extends string> =
  DotExtendedString<S> extends `${infer A}.${infer B}`
    ? A extends ''
      ? []
      : [A, ...PathArray<B>]
    : [];
 ...
 */
type NestedRecord<Path extends string[], K extends object = {}> = Path extends [
  infer Next,
  ...infer Rest,
]
 ...
 */
type DeepGet<K extends object, Path extends string> = _DeepGet<
  K,
  PathArray<Path>
>;

type _DeepGet<K extends object, T extends Array<string>> = T extends [
  infer First,
  ...infer Rest,
]
